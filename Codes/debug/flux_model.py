...
#                opti_df['dolo_blow_o'] = 0
            ...
            print('totals=',opti_df['dolo_total'].values[0], opti_df['lime_total'].values[0],opti_df['flux_total'].values[0] )
    ...
    #     cursor.execute(
    #     "INSERT INTO Flux_Model.dbo.flux_output (msg_time_stamp, heat, process_type, start_time, status_description, bathwt, hotmetal, grade, model_feasibility, dololime_setup_o, dolo_blow_o, lime_blowi_o, dolo_arc_o, lime_arc_o,dolo_total,lime_total,flux_total,lime_setup_o,hm_c,hm_si,hm_p,hm_mn,hm_s,dri_blow,scrap,cao_lime_perc,mgo_lime_perc,cao_dololime_perc,mgo_dololime_perc,cao_dri_perc,mgo_dri_perc,sio2_dri_perc,p_dri_perc,al2o3_dri_perc,basicity,mgo_perc) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
    #      row['msg_time_stamp'],
    #      row['heat'],
    #      row['process_type'],
    #      row['start'],
...
def main():
    ...
    conarc_flux_data= pd.read_sql("SELECT TOP(1000) * FROM conarc_flux_data where HEAT_NUMBER = '22303039' and  HM_WT >0 order by MSG_TIME_STAMP desc", conn)
    ...
    heat_status_check=heat_status_check[heat_status_check['HEAT_NUMBER']=='22303039']
