...
#Linear Equations to calculate Phosphorus value for each set created using Random Forest
...
def phos_pred_func(heat):
    ...
    #Creating arcing flag based on hm wt
    ads['Updated Arcing Flag']=0
    ads['Updated Arcing Flag'] = np.where((ads['HotMetal'] > hm_wt_check), 0,1)
    ...
    #getting latest values from signals to feed into prediction model
    try:
        ...
        if hm_wt_check<160:
            ...
            elif 'EAF 2nd Blowing End' in ads['status_description'].unique():
                blow_data=ads[(ads['status_description']=='EAF 2nd Blowing End')].sort_values('msg_time_stamp', ascending = False).drop_duplicates(subset=['heat'])[['heat','dri','dolo','lime','updated_arcing_flag']]
            elif 'EAF 3rd Blowing End' in ads['status_description'].unique():
                blow_data=ads[(ads['status_description']=='EAF 3rd Blowing End')].sort_values('msg_time_stamp', ascending = False).drop_duplicates(subset=['heat'])[['heat','dri','dolo','lime','updated_arcing_flag']]
            else:
            ...
            elif 'EAF 2nd Arcing Start' in ads['status_description'].unique():
                ads=ads[(ads['status_description']=='EAF 2nd Arcing Start')].sort_values('msg_time_stamp', ascending = False).drop_duplicates(subset=['heat'])
            elif 'EAF 3rd Arcing Start' in ads['status_description'].unique():
                ads=ads[(ads['status_description']=='EAF 3rd Arcing Start')].sort_values('msg_time_stamp', ascending = False).drop_duplicates(subset=['heat'])
            else:
        ...
        ads=ads[['heat','msg_time_stamp','shell','hotmetal','bathwt','grade','coke_f','status_description','hm_c_tons','hm_si_tons','hm_s_tons','hm_p_tons','hm_mn_tons']]
        ...
        ads['dri_arc'][ads['updated_arcing_flag']==1]=bath_wt_assumed-ads['hotmetal']-ads['dri_blow']


        querry_2="SELECT * FROM flux_output where heat = " + heat

        #Since arcing is not finished, we use Flux model predictions for dolo, lime qty at the end of arcing process
        ...
        #To get oxygen related data, we connect to Pi Server
        ...
        #Creating list of required features to run predictions
        list_features = ['heat',
                         'msg_time_stamp',
                         'shell',
                         'hotmetal',
        ...
        predicted_df=ads.copy()
        predicted_df['status_description']='P Predictions'
        now = datetime.datetime.now()
        now = now.strftime('%Y-%m-%d %H:%M:%S')
        predicted_df['msg_time_stamp'] = now
        predicted_df['model_feasibility']='infeasible'
        predicted_df['dololime_setup_o']=0
        predicted_df['dolo_blow_o']=0
        predicted_df['lime_blowi_o']=0
        predicted_df['lime_arc_o']=0
        predicted_df['dolo_arc_o']=0
        predicted_df['lm_p_predicted_o']=0
        ads['o2decarb']=0
        ads['o2toplance']=0
        ads['toplanceflowrate']=0
    ...
    for index, row in predicted_df.iterrows():
        ...
        cursor.execute(
        "INSERT INTO Flux_Model.dbo.flux_output (msg_time_stamp, heat, process_type, start_time, status_description, bathwt, hotmetal, grade, model_feasibility, dololime_setup_o, dolo_blow_o, lime_blowi_o, dolo_arc_o, lime_arc_o, p_prediction, o2decarb, o2toplance, toplanceflowrate,msg_flag) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
         row['now'],
         row['heat'],
         '',
         row['msg_time_stamp'],
    ...
    for index, row in predicted_df.iterrows():
        ...
        h_num=row['heat']
        SQLCommand = ("UPDATE Flux_Model.dbo.INT_JSW_FLUX_PHOS SET p_pre=?,msg_time_stamp=?,msg_flag=?  WHERE heat_number ="+ h_num+" ")
        cursor.execute(SQLCommand,row['lm_p_predicted_o'],row['msg_time_stamp'],'N')

#         cursor.execute(
#         "INSERT INTO Flux_Model.dbo.INT_JSW_FLUX_PHOS ( heat_number, p_pre, msg_flag, aggregate, msg_time_stamp) values (?,?,?,?,?)",
#          #row['msg_time_stamp'],
# #         now,
#          row['heat'],
#          row['lm_p_predicted_o'],
...
def main():
    ...
    # conarc_flux_data=pd.read_sql("SELECT * FROM conarc_flux_data where HEAT_NUMBER in ('22400877') and HM_WT >0", conn)
    ...
    for index, row in heat_status_check.iterrows():
        ...
        if model_run == 3:
            print('checking weight for heat', heat)
            querry="SELECT * FROM conarc_flux_data where HEAT_NUMBER = " + heat +" "
            p_pred_data=pd.read_sql(querry, conn)


            bath_wt_check=p_pred_data.sort_values('MSG_TIME_STAMP', ascending = False).drop_duplicates(subset=['HEAT_NUMBER'])['BATH_WT'].iloc[0]
            bath_wt_check=bath_wt_check/1000

            hm_wt=p_pred_data.sort_values('MSG_TIME_STAMP', ascending = False).drop_duplicates(subset=['HEAT_NUMBER'])['HM_WT'].iloc[0]
            hm_wt=hm_wt/1000

            print('hm_wt=',hm_wt)
            print('bath_wt_check=',bath_wt_check)

            #For Conarc Process, P Prediction to be run when bath wt reaches 210 T
            if hm_wt < 160:
                if bath_wt_check >=210:
                    phos_pred_func(heat)
                    print("phos prediction for heat -", heat)
            #For Fulll Blow Process, P Prediction to be run when bath wt reaches 205 T
            else:
                if bath_wt_check >=205:
                    phos_pred_func(heat)
                    print("phos prediction for heat -", heat)
