...
def optimization_func(running_heats, model_run):
    ...
    try:
        ...
        if len(dri_chem_data) == 0:
            dri_chem_data=material_chem_data[(material_chem_data['MAT_TYPE']=='DRI')].sort_values(['INSERT_DATE'],ascending=False)
            dri_chem_data['CAO_PER']=1.72
            dri_chem_data['MGO_PER']=0.68
            dri_chem_data['SIO2_PER']=5.16
            dri_chem_data['AL2O3_PER']=2.74
    ...
    #        model_run=row['model_run_number']
        ...
        try:
            ...
            print('lime blow 1 ',opti_df['lime_blowi_o'].values[0])
            ...
            print('lime setup ',opti_df['lime_setup_o'].values[0])
            print('lime blow 2 ',opti_df['lime_blowi_o'].values[0])
    ...
    #     cursor.execute(
    #     "INSERT INTO Flux_Model.dbo.flux_output (msg_time_stamp, heat, process_type, start_time, status_description, bathwt, hotmetal, grade, model_feasibility, dololime_setup_o, dolo_blow_o, lime_blowi_o, dolo_arc_o, lime_arc_o,dolo_total,lime_total,flux_total,lime_setup_o) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
    #      row['msg_time_stamp'],
    #      row['heat'],
    #      row['process_type'],
    #      row['start'],
    ...
    print(opti_df)#.to_csv('opti_output_15Nov.csv')
...
def main():
    ...
    conarc_flux_data= pd.read_sql("SELECT TOP(1000) * FROM conarc_flux_data where HEAT_NUMBER = '22402625' and  HM_WT >0 order by MSG_TIME_STAMP desc", conn)
    ...
    heat_status_check=heat_status_check[heat_status_check['HEAT_NUMBER']=='22402625']
