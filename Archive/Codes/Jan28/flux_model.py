...
def optimization_func(running_heats, model_run):
    ...
    print(cao_lime_perc,cao_dololime_perc,mgo_lime_perc,mgo_dololime_perc)
...
#        model_run=row['model_run_number']
        ...
        if model_run == 1:
            ...
            row['blowing_cao_actual']=row['lime_blowi'] * cao_lime_perc + row['dolo_blow'] * cao_dololime_perc + row['dri_blow'] * 1000 * cao_dri_perc
            row['blowing_cao_delta']=row['blowing_cao']-row['blowing_cao_assumed']

            row['blowing_mgo_actual']=row['lime_blowi'] * mgo_lime_perc + row['dolo_blow'] * mgo_dololime_perc + row['dri_blow'] * 1000 * mgo_dri_perc
            row['blowing_mgo_delta']=row['blowing_mgo']-row['blowing_mgo_assumed']
...
def main():
    ...
    conarc_flux_data= pd.read_sql("SELECT TOP(1000) * FROM conarc_flux_data where HEAT_NUMBER = '22200693' and  HM_WT >0 order by MSG_TIME_STAMP desc", conn)
    ...
    heat_status_check=heat_status_check[heat_status_check['HEAT_NUMBER']=='22200693']
