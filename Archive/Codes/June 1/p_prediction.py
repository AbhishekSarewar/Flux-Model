...
def main():
    ...
    #print(heat_status_check)
    #heat_status_check=heat_status_check[heat_status_check['HEAT_NUMBER']=='22400876']
    #print(heat_status_check)
    ...
    for index, row in heat_status_check.iterrows():
        ...
        if model_run == 3:
            ...
            check_p = 0
            if hm_wt < 160:
                if len(p_pred_data[(p_pred_data['STATUS_DESCRIPTION']=='EAF 1st Arcing Start') | (p_pred_data['STATUS_DESCRIPTION']=='EAF 2nd Arcing Start')].sort_values('MSG_TIME_STAMP', ascending = True).drop_duplicates(subset=['HEAT_NUMBER'])['MSG_TIME_STAMP'])>0:
                    ...
                    start_time=p_pred_data[(p_pred_data['STATUS_DESCRIPTION']=='EAF 1st Arcing Start') | (p_pred_data['STATUS_DESCRIPTION']=='EAF 2nd Arcing Start')].sort_values('MSG_TIME_STAMP', ascending = True).drop_duplicates(subset=['HEAT_NUMBER'])['MSG_TIME_STAMP'].iloc[0]

                    dri_blow=p_pred_data[(p_pred_data['STATUS_DESCRIPTION']=='EAF 1st Blowing End') | (p_pred_data['STATUS_DESCRIPTION']=='EAF 2nd Blowing End') | (p_pred_data['STATUS_DESCRIPTION']=='EAF 3rd Blowing End')].sort_values('MSG_TIME_STAMP', ascending = False).drop_duplicates(subset=['HEAT_NUMBER'])['DRI_KG'].iloc[0]
                    #flux_output_data[(flux_output_data['heat']==heat) & (flux_output_data['status_description']=='Arcing Predictions')]
                    print(hm_wt)
                    dri_arc=225-hm_wt-dri_blow/1000
