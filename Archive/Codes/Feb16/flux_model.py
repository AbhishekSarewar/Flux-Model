...
def optimization_func(running_heats, model_run):
    ...
    ads['Updated Arcing Flag'] = np.where((ads['HotMetal'] > full_blow_wt_check), 0,1)
...
#        model_run=row['model_run_number']
        ...
        if model_run == 1:
            ...
            row['blowing_cao_actual']=row['lime_blowi'] * cao_lime_perc*1000 + row['dolo_blow'] * cao_dololime_perc*1000 + row['dri_blow'] * 1000 * cao_dri_perc
            row['blowing_cao_delta']=row['blowing_cao']-row['blowing_cao_actual']

            print('blowing_cao', row['blowing_cao'])
            print('blowing_cao_actual', row['blowing_cao_actual'])
            print('blowing_cao_delta', row['blowing_cao_delta'])

            row['blowing_mgo_actual']=row['lime_blowi'] * mgo_lime_perc*1000 + row['dolo_blow'] * mgo_dololime_perc *1000+ row['dri_blow'] * 1000 * mgo_dri_perc
            row['blowing_mgo_delta']=row['blowing_mgo']-row['blowing_mgo_actual']

            print('blowing_mgo', row['blowing_mgo'])
            print('blowing_mgo_actual', row['blowing_mgo_actual'])
            print('blowing_mgo_delta', row['blowing_mgo_delta'])
    ...
    #     for index, row in opti_df.iterrows():
    #         cursor.execute(
    #         "INSERT INTO Flux_Model.dbo.INT_JSW_FLUX_Pre (update_date, heat_number, dolo_setup, dolo_blow, lime_blow, dolo_arc, lime_arc,  msg_flag, aggregate, msg_time_stamp) values (?,?,?,?,?,?,?,?,?,?)",
    #          row['msg_time_stamp'],
    #          row['heat'],
    #          row['dololime_setup_o'],
    #          row['dolo_blow_o'],
    ...
    #     cursor = conn.cursor()
    #
    #     seq_value_arc= pd.read_sql("SELECT max(seq_value) From INT_JSW_FLUX_Pre", conn).values[0]
    #
    #     for index, row in opti_df.iterrows():
    #         cursor.execute(
    #         "INSERT INTO Flux_Model.dbo.INT_JSW_FLUX_ARC (update_date, heat_number, dolo_setup, dolo_blow, lime_blow, dolo_arc, lime_arc,  msg_flag, aggregate, msg_time_stamp) values (?,?,?,?,?,?,?,?,?,?)",
    #          row['msg_time_stamp'],
    #          row['heat'],
    #          row['dololime_setup_o'],
    #          row['dolo_blow_o'],
...
def main():
    ...
    conarc_flux_data= pd.read_sql("SELECT TOP(1000) * FROM conarc_flux_data where HEAT_NUMBER = '22200833' and  HM_WT >0 order by MSG_TIME_STAMP desc", conn)
    ...
    #heat_status_check[['HEAT_NUMBER','model_run']]
    heat_status_check=heat_status_check[heat_status_check['HEAT_NUMBER']=='22200833']
    ...
    # running_heats=conarc_flux_data[conarc_flux_data['HEAT_NUMBER']==heat]
        ...
        if len(running_heats)>0:
            ...
            #optimization_func(running_heats, 2)
